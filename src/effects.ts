import { parseNetLibramEffects } from './parseUtils';
...
export function parseEffectsString(data: string): Record<number, string> {
    ...
    console.log(`Parsing ${lines.length} lines of effect data`);
    
    // Try to detect if the file contains Windows-style line endings
    const hasWindowsLineEndings = data.includes('\r\n');
    if (hasWindowsLineEndings) {
        console.log("Detected Windows-style line endings (\\r\\n)");
    }
    
    // Try to detect if there are invisible characters at the start of lines
    if (lines.length > 0 && lines[0].charCodeAt(0) > 127) {
        console.log(`Warning: First line starts with non-ASCII character: ${lines[0].charCodeAt(0)}`);
    }
    ...
    for (const line of lines) {
        // First try to detect any unusual characters in the line
        let cleanLine = line;
        
        // Remove any BOM or other special characters
        if (cleanLine.charCodeAt(0) === 0xFEFF || cleanLine.charCodeAt(0) > 127) {
            cleanLine = cleanLine.slice(1);
        }
        
        // Replace any weird whitespace with normal spaces
        cleanLine = cleanLine.replace(/\\s+/g, ' ');
        
        // Try a more lenient approach to match the effect number at the start
        // This regex looks for 1-4 digits at the beginning of the line, followed by whitespace
        const match = cleanLine.match(/^(\d{1,4})\\s+(.+)$/);
        ...
        if (match) {
            ...
            // Log some sample parsing for debugging
            if (successCount <= 5 || successCount % 1000 === 0) {
                console.log(`Parsed effect #${effectNumber}: ${effectDescription.substring(0, 30)}...`);
            }
        } else {
            failCount++;
            // Try a more aggressive approach
            const numbersOnly = cleanLine.match(/^(\d+)/);
            if (numbersOnly) {
                const effectNumber = parseInt(numbersOnly[1], 10);
                const effectDescription = cleanLine.substring(numbersOnly[0].length).trim();
                
                if (effectDescription) {
                    console.log(`Recovered effect #${effectNumber} using fallback method`);
                    effects[effectNumber] = effectDescription;
                    successCount++;
                } else {
    ...
    console.log(`Successfully parsed ${Object.keys(effects).length} effects (${successCount} success, ${failCount} failed)`);
...
export async function parseEffectsFile(file: File): Promise<Record<number, string>> {
    try {
        ...
        // First try our specialized NetLibram parser
        const effects = parseNetLibramEffects(text);
        
        // If it parsed at least some effects, return them
        const effectCount = Object.keys(effects).length;
        if (effectCount > 0) {
            console.log(`Successfully parsed ${effectCount} effects using specialized parser`);
            return effects;
        }
        
        // Fall back to the older parser
        console.log("Specialized parser didn't find any effects, trying fallback parser");
